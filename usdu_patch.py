...
- Adds guider support for batch tile processing
"""
...
def patched_script_run(self, p, _, tile_width, tile_height, mask_blur, padding, seams_fix_width, seams_fix_denoise, seams_fix_padding,
    ...
    logger.debug("Patched script.run() preserving batch_size=%s", preserved_batch_size)
    ...
    logger.debug("Patched script.run() complete, batch_size=%s", p.batch_size)
...
def _prepare_tile_for_batch(calc_rectangle_fn, current_image: Image.Image, tx: int, ty: int, p) -> Tuple[Image.Image, Tuple[int, int, int, int], Image.Image, Tuple[int, int]]:
    ...
    Returns: (cropped_tile, initial_tile_size, tile_mask, crop_region, tile_size)
    """
...
def _process_batch_tiles(p,
    ...
    # CRITICAL: guider check must encompass conditioning crop AND sampling
    # p.positive/p.negative are None for guider processing - accessing them crashes
    if getattr(p, 'use_guider', False):
        if len(tiles_coords) > 1:
            logger.warning("Guider with batch_size > 1: conditioning broadcasts across %d tiles", len(tiles_coords))
...
def patch_usdu_linear_and_chess_process():
    ...
    def new_linear_process(self, p, image, rows, cols):
        ...
        logger.debug("linear_process called batch_size=%s rows=%s cols=%s total_tiles=%s", batch_size, rows, cols, rows * cols)

        if batch_size <= 1:
            logger.debug("Using original single-tile processing (batch_size=%s)", batch_size)
            return old_linear(self, p, image, rows, cols)

        # Batch mode
        vae_encoder = p.vae_encoder
        vae_decoder = p.vae_decoder
        vae_decoder_tiled = p.vae_decoder_tiled
        ...
        for yi in range(rows):
            for xi in range(cols):
                ...
                if len(tiles_to_process) >= batch_size or (yi == rows - 1 and xi == cols - 1):
                    batch_count += 1
                    logger.debug("Processing batch #%s with %s tiles: %s", batch_count, len(tiles_to_process), tiles_to_process)
        ...
        logger.debug("Linear processing complete. Processed %s batches total.", batch_count)
...
logger.debug("USDU batch patches applied successfully.")
